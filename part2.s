/******************************************************************************
* file: part_2.s (Sub Routines, Binary search)
* author: Parth Sah
...
ary_size: .word 0
src_num: .word 0
msg: .asciz "\nEnter numbers of Array in assending order:\n"
...
Find:
	...
	mov r6, #0          @ initialize r6 to 0  min position
	sub r7, r4, #1		@ initialize r7 with last position of array max position
search:
	add r8, r6, r7				@ add mix and max
	mov r8, r8, lsr #1			@ min+max/2 to find the center element
	ldr r1, [r3, r8, lsl #2]	@ read center element
	cmp r5, r1					@ compare center element with search element
	mov r2, r8					@ update search position
	beq Matchfound				@ if compare tell equal, brach to Matchfound
	bgt greater					@ if compare tell greater, brach to greater
	blt lesser					@ if compare tell lesser, brach to lesser
cont:
	cmp r6, r7					@ check to see if min and max positions converge at a single position
	bne search					@ if not branch to find the next 
	ldr r1, [r3, r6, lsl #2]	@ if yes, read the converge positoin
	cmp r1, r5					@ compare it with search element
	mov r2, r6					@ update search position
	beq Matchfound				@ if compare tell equal, brach to Matchfound
	bl Notfound					@ if not, we searched entire array, match not found, call Notfound
	ldm sp!, {lr}				@ pop lr from stack
	bx lr                       @ branch to lr
greater:
	add r6, r8, #1				@if search element if greater then center min position is center+1
	b cont
lesser:
	sub r7, r8, #1				@if search element if lesser then center max position is center-1
	b cont
Matchfound:
	stmdb sp!, {lr}	@ push lr to stack
